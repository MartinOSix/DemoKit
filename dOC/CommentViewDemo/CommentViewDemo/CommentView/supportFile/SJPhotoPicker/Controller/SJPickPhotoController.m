...
- (void)setupDatas {
    ...
    // pickedArray
    {
        ...
        if ([SJPhotoPicker shareSJPhotoPicker].defaultPHAsset) {
            for (PHAsset *aset in [SJPhotoPicker shareSJPhotoPicker].defaultPHAsset) {
                for (SJPhotoModel *model in self.photoArray) {
                    if ([aset.creationDate  isEqualToDate:model.asset.creationDate]) {
                        //SJPhotoModel *model = [[SJPhotoModel alloc] init];
                        model.asset = aset;
                        model.inAlbumIndex = [self.photoArray indexOfObject:model];
                        model.pickedIndex = self.pickedArray.count+1;
                        model.isPicked = YES;
