...
-(void)drawBackgroundColorRegion:(CGContextRef) ctx{
	...
	for (CSChartsColorRegion *colorRegion in colorRegionArray) {
		if (self.charts.background.isRegionSeparated) {
			...
			NSArray *colors = @[
								[UIColor colorWithCGColor:[[colorRegionArray objectAtIndex:0]  color]],
								[UIColor colorWithCGColor:[[colorRegionArray objectAtIndex:1] color]
								]];
...
-(void)drawBackgroundHorizontalLines:(CGContextRef) ctx{
    ...
    for (int i = 0;i < horizontalLineAmount - 1; i ++ , lineYPosition += horizontalSpacing) {
        //set line color
        if (i % 5 == 0) {
...
//        points[0] = CGPointMake(lineXPosition, topSpacing);
        points[0] = CGPointMake(lineXPosition, self.frame.size.height - 6 - bottomSpacing);

        points[1] = CGPointMake(lineXPosition, self.charts.frame.size.height - bottomSpacing - 1);
