...
- (void)pickBtnAction:(UIButton *)sender {
    ...
    if(self.model.asset.mediaType != PHAssetMediaTypeImage){
        
        [[CommentView cqGetController:self].view makeToast:@"只能选择图片类型文件"];
        return;
    }
    
    if (!self.model.isPicked) {
        if ([SJPhotoPicker shareSJPhotoPicker].selectedImageCount>=kImagesTotalCount) {
            [[CommentView cqGetController:self].view makeToast:@"选择图片数量已达上限"];
            return;
        }
        [SJPhotoPicker shareSJPhotoPicker].selectedImageCount += 1;
    }else{
        [SJPhotoPicker shareSJPhotoPicker].selectedImageCount -= 1;
    }
